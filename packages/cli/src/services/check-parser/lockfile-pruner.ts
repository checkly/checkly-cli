...
function parseLockfileSnapshot (content: string, lockfileName: string): LockfileSnapshot {
  ...
  if (lockfileName === 'pnpm-lock.yaml') {
    // pnpm 12 may prepend an environment document that pins pnpm itself. Both
    // the original and the regenerated lockfile go through here, and only the
    // application document is compared: the environment document's entries
    // are pnpm's own binaries, not resolutions the bundle depends on. The
    // regenerated bytes are still shipped whole, environment document included.
    const doc = parseYaml(splitPnpmLockfileDocuments(content).main)
