...
- The `request` object must include a `sslConfig` object. The `sslConfig.hostname` field is required (hostname only, no scheme).
- Use `sslConfig.alertDaysBeforeExpiry` to raise an alert before the certificate expires (default: 20 days).
- Use `sslConfig.degradedResponseTimeMs` and `sslConfig.maxResponseTimeMs` to configure TLS handshake time thresholds.
- Use `sslConfig.securityBaseline` to enforce TLS version, key size, cipher suite, and other certificate quality rules.
- For mutual TLS, set `sslConfig.clientCertificateMode` to `'auto'` or `'explicit'`. When `'explicit'`, also set `sslClientCertificateId`.
